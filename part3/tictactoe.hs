...
autoTurn forceRandom (count, player, game) = do
    ...
    -}
    -- Try a deterministic "random" position based on a cycle
    let positions = [0..8]
    let posIndex = (count + 1) `mod` length positions  -- Cycle through positions
    let i = positions !! posIndex

    -- Block the opponent or guess the best move
    let moveToMake = case canBlock player game of
            Nothing -> bestMoveFor player game
            Just blockPos -> blockPos
